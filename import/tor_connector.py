...
import sys, os, socket, logging, random
...
class TorConnector(Subject):
    ...
    @custom_circuit.setter
    def custom_circuit(self, value):
        self._custom_circuit = value
    ...
    def __init__(self, tor_number):
        self.tor_ctrl_pwd = secrets.TOR_CONTROL_PASSWORD
    ...
    def init_tor_process(self): 
        ...
        try:
            ...
            self.launch_tor(self._socks_port, self._control_port)
    ...
    def get_circuit_status(self, circuit):
        try:
            return self.controller.get_circuit(circuit).status
        except:
            return None
    ...
    def custom_circuit_launched(self, circuit):
        if self.custom_circuit == circuit.id:
            self.custom_circuit_status = circuit.status
            self.notify()

            if circuit.status == 'FAILED' or circuit.status == 'CLOSED':
                logging.debug("** Circuit build FAILED, trying again")
                try:
                    yield threads.deferToThread(self.create_circuit())
                except Exception as e:
                    logging.debug("Failed to create new circuit: {}".format(e))
                return
        ...
        if stream.status == 'NEW':
            ...
            if self.custom_circuit != None:
                try:
                    ...
                    print("Circuit %s is CLOSED, unable to attach stream %s" % self.custom_circuit, stream.id)
                if circuit.status == 'BUILT':
                    try:
                        self.controller.attach_stream(stream.id, circuit.id)
                        print("connecting stream with circuit %s" % circuit.id)
                    except Exception as exc:
                        print("couldn't attach stream to circuit {0} due to exception: {1}".format(circuit.id, exc))
    ...
    def create_circuit(self):
      ...
      #  self.controller.add_event_listener(self.custom_circuit_launched, 'CIRC')
        try:
            ...
            self.custom_circuit = new_circuit 
        except:
            raise CircuitCreationError()
        print("Created circuit %s" % self.custom_circuit)
